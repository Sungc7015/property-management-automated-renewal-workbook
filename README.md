...
|---|---|---|
| 2.10.1 | 2026-07-23 | Two bug fixes to the v2.10.0 Keep/Lock feature, found in real-Excel testing the same day. (1) Current Rent on a locked/out-of-window Pending row had no live refresh source and could stay permanently blank or stale once a unit was resurrected purely from its carried-forward snapshot — fixed by giving it the same fresh-lookup treatment Market Rent already had: `ReadYardiMTM` now also exposes a per-unit `allActualRent` dictionary (mirroring the existing `allMarketRent`), threaded through `RefreshMTMSheet` -> `DoRefreshMTM` -> `BuildPendingSection`, looked up fresh each refresh and falling back to the carried value only if the unit isn't in that refresh's Rent Roll. (2) Source Month's cell format was only being applied inside `WritePendingDataRow`'s `ParseMonthSheet` success branch, unlike every other formatted column in that sub which formats unconditionally - some rows could end up showing a raw date serial number (e.g. "46082") instead of "Mar-26"; moved the format assignment to the unconditional block. Both confirmed fixed in real Excel. |
| 2.10.0 | 2026-07-23 | Added a "Keep/Lock" checkbox (column J) to the Pending (Manual) MTM tracker section, fixing a bug where a unit manually flagged "MTM" on the current month's sheet could be silently dropped from Pending on refresh if the anchor's 3-month scan window hadn't caught up yet (the anchor only advances on a formal monthly import, not the real calendar month). Checking a row's Keep/Lock box keeps it in Pending even if it falls outside the scan window — and, per user decision, even after it graduates to a real Confirmed row (an intentional duplicate listing across both sections) — only an explicit resolution (e.g. marked "Renewed") on a later sheet removes a locked row. Implemented via the same Form Control checkbox pattern already used by Confirmed's Import checkboxes (`SyncPendingCheckboxes`, modeled on `SyncCheckboxes`), and the Keep/Lock value survives every Pending rebuild via the existing carry-forward snapshot mechanism. Also fixed `modConfig.VER` itself, which had drifted to "2.8.0" since the 2.9.0 release's version-bump commit only updated the README banner, not the constant. Tested and confirmed working in real Excel. |
| 2.9.0 | 2026-07-07 | Pending (Manual) expanded from 5 to 9 columns: Expected Increase Date and New MTM Rate (manually maintained, carried forward every refresh; New MTM Rate is also picked up live from month sheets' col S "MTM Rate" the instant col A is set to "MTM"), plus Market Rent (now auto-imported from the Rent Roll import rather than manual, via a new unit→rate lookup built alongside the existing MTM-only Rent Roll read). Next Increase (Confirmed) and Expected Increase Date (Pending) get a soft highlight fill so these dates stand out. Added a static usage-reminder note on the sheet. "Sort MTM Tracker" now sorts the live `MTMTable` Excel Table via its own `Sort` object instead of a raw `Range.Sort` (which could reset the header's formatting/position), and no longer recomputes Next Increase before sorting - it's carried forward untouched on both sort and refresh, only computed fresh when a unit has none on record. `NextIncreaseDate` fixed to exactly 12 months (`=EDATE(...,12)`, was previously +1 year +1 day). `BuildPendingSection`'s 3-month window rescan now drops a unit whose later month sheet shows an explicit non-MTM status, instead of leaving it in Pending indefinitely, and also drops any unit already written as a real Confirmed row this refresh (so a unit doesn't reappear in Pending after being handled above) - when that happens, its Expected Increase Date carries into Next Increase and its New MTM Rate/Notes fold into Confirmed's Notes so nothing is lost on the transition. Two independent Fable-model audit passes (no VBA runtime available to verify this kind of change) found and fixed 5 bugs across this work, including a Type Mismatch on omitted Optional arguments, a snapshot-taken-after-delete ordering bug, and a stale-variable bug that could leak one unit's Market Rent onto another. |
| 2.8.0 | 2026-07-07 | Added a "Pending (Manual)" section to the MTM tracker sheet — surfaces units manually flagged "MTM" in month sheets' Renewal Status column (col A) ahead of the rent roll confirming it. An anchor cell (`MTM.AnchorDate` named range) auto-updates to the last-imported month/year (`modImport.ImportMonthlyData` → `modMTM.SetMTMAnchor`); a rolling 3-month window (`modSheetUtils.PendingWindowSheets`) is the only range ever scanned for "MTM" statuses, so old years' month sheets (which are never archived) can't resurface stale statuses. A live `Worksheet_Change`-style trigger (`modDynamic.HandlePendingStatusChange`) copies a unit into Pending the instant col-A is set to "MTM"; `RefreshMTMSheet`/`DoRefreshMTM` rebuild Pending from scratch each refresh as the authoritative source (drops units no longer flagged), both paths deduping by trimmed Unit#. Confirmed section's business logic is unchanged, but its row-bounding switched from a blind column-A `End(xlUp)` scan to `FindConfirmedLastRow` (which locates the new Pending divider row) across `ImportSelectedMTM`, `ClearSelectionMTM`, `SortMTMSheet`, and `SyncCheckboxes`, so Confirmed-section operations never touch Pending rows below them. Sheet auto-creation centralized in `CreateFreshMTMSheet` (used by `RefreshMTMSheet`, `SetMTMAnchor`, `AddPendingUnit`) with a `suppressActivate` option so the feature doesn't steal the user's screen focus mid-edit/mid-import. Source Month column stores a real Date value (`mmm-yy` format) instead of raw sheet-name text. Fixed post-implementation via a 3-model audit (Fable/Opus/Sonnet): an off-by-one in `FindConfirmedLastRow` that could corrupt a blank spacer row via "Sort MTM Tracker", a stale `#REF!` anchor-name crash risk if the MTM sheet is deleted/recreated, and legacy month-sheet name (e.g. "March 2026") exclusion from the window check. |
